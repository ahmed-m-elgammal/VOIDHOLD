...
var next_guid: int = 1
# Epic 2 world-layer state (T0.6 dome record: guid,topleft,width,...).
# Defaults keep Epic 1 parity: full-grid dome at origin, west entrance.
var topleft: Vector2i = Vector2i.ZERO
var dome_size: int = 80
var entrance_side: int = 3
...
func energy_saturation() -> float:
        ...
        var mult: float = tune_float(["solar_mult"], 1.0)
        var solar: float = 0.0
        var need: float = 0.0
        for b in buildings:
                if b is Dictionary == false:
                ...
                if kind == "solar":
                        solar += (1.0 - float(bd.get("damage", 0.0))) * per * mult
