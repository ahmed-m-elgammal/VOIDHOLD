...
var _sky_mat: PanoramaSkyMaterial = null
...
func _ready() -> void:
	...
	if env != null and env.environment != null:
		if env.environment.sky != null and env.environment.sky.sky_material is PanoramaSkyMaterial:
			_sky_mat = env.environment.sky.sky_material as PanoramaSkyMaterial
...
func _apply_visuals(sun_h: float) -> void:
	...
	if want_night != _night_showing and _sky_mat != null and _tex_night != null:
