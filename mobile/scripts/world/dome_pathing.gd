...
func resync() -> void:
        ...
        # Elevator doors stay walkable too (T2.4: the lift must remain
        # reachable from the dome interior even on rocky door terrain).
        if dome != null:
                dome.sync_indices()
                for b in dome.by_type.get("elevator", []):
                        if b is Dictionary == false:
                                continue
                        var door_v: Variant = (b as Dictionary).get("door", Vector2i(-1, -1))
                        if door_v is Vector2i == false:
                                continue
                        var door: Vector2i = door_v
                        if rect.has_point(door):
                                _astar.set_point_solid(door, false)
