...
# The elevator sits at the middle of one edge; the rect extends away from it.
# Returns Rect2i() when no non-overlapping, in-bounds, walkable candidate fits.
func find_new_dome_rect(dome: DomeSim, elevator_cell: Vector2i, size: int = WorldTheme.DOME_SIZE) -> Rect2i:
	var ex: int = elevator_cell.x
	var ey: int = elevator_cell.y
	var half: int = size / 2 - 2
	var candidates: Array = [
		{"rect": Rect2i(Vector2i(ex - half, ey + 4), Vector2i(size, size)), "side": WorldTheme.SIDE_NORTH},
		{"rect": Rect2i(Vector2i(ex - half, ey - size - 4), Vector2i(size, size)), "side": WorldTheme.SIDE_SOUTH},
		{"rect": Rect2i(Vector2i(ex + 4, ey - half), Vector2i(size, size)), "side": WorldTheme.SIDE_WEST},
		{"rect": Rect2i(Vector2i(ex - size - 4, ey - half), Vector2i(size, size)), "side": WorldTheme.SIDE_EAST},
	]
