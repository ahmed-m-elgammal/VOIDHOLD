...
func _test_scene_integration() -> void:
        ...
        if elev_cell.x >= 0:
                ...
                _test_elevator_nav_reachable(manager, dome2)
...
# ---------------------------------------------------------------- T2.4 nav reachability

# Walkable cell nearest to `center` according to the dome's live pathing.
func _nearest_walkable_in(dw: DomeWorld, center: Vector2i) -> Vector2i:
        var best := Vector2i(-1, -1)
        var best_d: int = 2147483647
        for x in range(dw.rect.position.x, dw.rect.end.x):
                for y in range(dw.rect.position.y, dw.rect.end.y):
                        var c := Vector2i(x, y)
                        if dw.pathing.is_walkable(c) == false:
                                continue
...
func _test_elevator_nav_reachable(manager: DomeManager, dome2: DomeSim) -> void:
        # The new dome's elevator must be a real part of the dome: footprint
        # inside the rect, door pathable into the interior, entrance gap
        # pathable to both the interior and the lift door.
        var dw2: DomeWorld = manager.world_at(1)
        if dw2 == null:
                check("elevator nav: world exists", false)
                return
        dome2.sync_indices()
        var lifts: Array = dome2.by_type.get("elevator", [])
        check("elevator nav: lift record present", lifts.is_empty() == false)
        if lifts.is_empty():
                return
        var lift: Dictionary = lifts[0]
        var lcell: Vector2i = lift.get("cell", Vector2i(-9, -9))
        var lsize := Vector2i(int(lift.get("w", 0)), int(lift.get("h", 0)))
        check("elevator nav: footprint inside dome2 rect", dw2.rect.encloses(Rect2i(lcell, lsize)))
        var door2: Vector2i = lift.get("door", Vector2i(-9, -9))
        check("elevator nav: door inside dome2 rect", dw2.rect.has_point(door2))
        check("elevator nav: door walkable in pathing", dw2.pathing.is_walkable(door2))
        var center2: Vector2i = dw2.rect.position + dw2.rect.size / 2
        var target: Vector2i = _nearest_walkable_in(dw2, center2)
        check("elevator nav: interior walkable cell exists", target.x >= 0)
        if target.x < 0:
                return
        check("elevator nav: door paths to interior", dw2.pathing.path_exists(door2, target))
        var cells: Array = dw2.pathing.entrance_cells
        check("elevator nav: entrance cells present", cells.is_empty() == false)
        if cells.is_empty():
                return
        var e0 := Vector2i(int((cells[0] as Vector2i).x), int((cells[0] as Vector2i).y))
        check("elevator nav: entrance paths to interior", dw2.pathing.path_exists(e0, target))
        check("elevator nav: entrance paths to lift door", dw2.pathing.path_exists(e0, door2))
